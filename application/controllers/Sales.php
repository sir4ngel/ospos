...
class Sales extends Secure_Controller
{
	...
	public function complete()
	{
		...
		if($this->sale_lib->is_invoice_mode())
		{
			...
			else
			{
				...
				$data['sale_id_num'] = $this->Sale->save($sale_id, $data['sale_status'], $data['cart'], $customer_id, $employee_id, $data['comments'], $invoice_number, $stock_location, $work_order_number, $quote_number, $sale_type, $data['payments'], $data['dinner_table'], $tax_details);
	...
	public function send_pdf($sale_id, $type = 'invoice')
	{
		...
		if(!empty($sale_data['customer_email']))
		{
			...
			$tokens = array(new Token_invoice_sequence($sale_data['invoice_number']),
				new Token_invoice_count( $sale_data['sale_id']),
	...
	private function _load_sale_data($sale_id)
	{
		...
		$data['show_stock_locations'] = $this->Stock_location->show_locations('sales');
		$data['stock_location'] = $sale_info['stock_location'];
	...
	public function save($sale_id = -1)
	{
		...
		$this->Inventory->update($sale_id, ['trans_date' => $sale_time]);
	...
	public function suspend()
	{
		...
		if($this->Sale->save($sale_id, $sale_status, $cart, $customer_id, $employee_id, $comment, $invoice_number, $stock_location, $work_order_number, $quote_number, $sale_type, $payments, $dinner_table, $sales_taxes) == '-1')
		{
