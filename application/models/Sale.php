...
class Sale extends CI_Model
{
	...
	public function get_info($sale_id)
	{
		...
		$this->db->select('
				...
				MAX(sales.stock_location) AS stock_location,
	...
	public function is_valid_receipt(&$receipt_sale_id)
	{
		if(!empty($receipt_sale_id))
		{
			...
			elseif($this->config->item('invoice_enable') == TRUE)
			{
				...
				{
					$receipt_sale_id =  $sale_info->row()->sale_id;
	 ...
	 */
	public function save($sale_id, &$sale_status, &$items, $customer_id, $employee_id, $comment, $invoice_number, $stock_location, $work_order_number, $quote_number, $sale_type, $payments, $dinner_table, &$sales_taxes)
	{
		if($sale_id != -1)
		{
